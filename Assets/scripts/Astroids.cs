...
{    
    ...
    public GameObject SecondAstroid;
    ...
    void Update()
    {
        ...
        if (health <= 0)
        {
            ...
            Destroy(gameObject);
            FindObjectOfType<Score>().IncScore_big();
            Instantiate(explosion, new Vector3(transform.position.x, transform.position.y, transform.position.z), Quaternion.identity);
            Instantiate(SecondAstroid, new Vector3(transform.position.x, transform.position.y, transform.position.z),  Quaternion.identity);
            Instantiate(SecondAstroid, new Vector3(transform.position.x, transform.position.y, transform.position.z), Quaternion.identity);
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("shielditself"))
        {
            FindObjectOfType<AudioManager>().playsound("AstroidExplosion");
            Destroy(gameObject);
            GameObject shielditself = collision.gameObject;
